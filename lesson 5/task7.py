...
prof_averege = 0
i = 0
with open('test_file_task7.txt') as file:
    for line in file:
    ...
    if i != 0:
        prof_averege = prof / i
        print(f'Средняя прибыль: - {prof_averege}')
    else:
        print(f'Средняя прибыль отсутсвует.')
    pr = {'average_profit': prof_averege}
    profit.update(pr)
    print(f'Прибыль каждой компании - {profit}')

with open('test_file_task7.json', 'w') as write_json:
    json.dump(profit, write_json)

    json_str = json.dumps(profit)
    print(f'Cодержимое списка сериализовано в файл json: test_file_task7.json')